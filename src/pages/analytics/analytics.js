...
for (let i = 0; i < 7; i++) {
    constants.daysAccordance[6 - i].classList.remove('showUp');
...
.then(() => {
    ...
    constants.analyticsNumbers[0].classList.add('showUp');
    ...
    constants.analyticsNumbers[1].classList.add('showUp');
    constants.titleAnalytics.textContent = `«${sessionStorage.getItem('input')}»`;
    constants.titleAnalytics.classList.add('showUp');
    return constants.analyticsNumbers[1].textContent;
})
.then((totalAccordance) => {
    for (let i = 0; i < 7; i++) {
        ...
        constants.daysWeek[6 - i].classList.add('showUp');
        newsApi.getNews(sessionStorage.getItem('input'), i, i)
        .then((data) => {
            constants.daysAccordance[6 - i].classList.add('growUp');
    ...
    constants.graphicMonth.classList.add('showUp');
})
.catch(error => {
